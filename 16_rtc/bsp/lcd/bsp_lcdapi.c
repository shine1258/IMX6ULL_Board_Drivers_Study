...
void lcd_drawline(unsigned short x1, unsigned short y1, unsigned short x2, unsigned short y2)
{
    ...
    for (t = 0; t <= distance + 1; t++) /* 画线输出 */
    {
        ...
        lcd_draw_pixel(uRow, uCol, lcd.foreground); /* 画点 */
        xerr += delta_x;
        yerr += delta_y;
        if (xerr > distance) {
            xerr -= distance;
...
void lcd_draw_Circle(unsigned short x0, unsigned short y0, unsigned char r)
{
    ...
    {
        lcd_draw_pixel(x + mx, y + my, lcd.foreground);
        lcd_draw_pixel(y + mx, x + my, lcd.foreground);
        lcd_draw_pixel(-x + mx, y + my, lcd.foreground);
        lcd_draw_pixel(-y + mx, x + my, lcd.foreground);
    ...
    unsigned char mode)
{
    ...
    for (t = 0; t < csize; t++) {
        ...
        for (t1 = 0; t1 < 8; t1++) {
            if (temp & 0x80)
                lcd_draw_pixel(x, y, lcd.foreground);
            else if (mode == 0)
                lcd_draw_pixel(x, y, lcd.background);
            temp <<= 1;
            y++;
            if (y >= lcd.height)
                return; /* 超区域了 */
